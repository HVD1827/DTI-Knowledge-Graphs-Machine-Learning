...
def convert_drug_drug_similarities_to_matrix(path):
    with open(path) as f:
        data = f.readlines()
    ...
    with open("./data/input_embedding_dict/ic/drug.txt", "w") as f:
...
def convert_target_target_similarities_to_matrix(path):
    with open(path) as f:
        data = f.readlines()
    ...
    # Afficher la matrice d'adjacence avec les noms de médicaments et de cibles
    with open("./data/input_embedding_dict/ic/target.txt", "w") as f:
        # Écrire les noms de cibles dans la première ligne
        f.write("\t" + "\t".join(targets) + "\n")

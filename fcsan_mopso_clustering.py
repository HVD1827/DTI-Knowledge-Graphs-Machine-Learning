...
def fcan_mopso_clustering(data, n_clusters, max_iter,c1,c2,w):
    from pyswarms.single.global_best import GlobalBestPSO
    ...
    n_particles = 10
    init_pos = np.tile(np.array(initial_centers).flatten(), (n_particles, 1))
    noise = np.random.normal(scale=0.1, size=init_pos.shape)
    init_pos += noise 
    ...
    def cost_function(positions):
        ...
        for pos in positions:
            centers = pos.reshape(n_clusters, n_features)
            intra = np.sum([np.mean(np.linalg.norm(data - c, axis=1)) for c in centers])
            inter = 1 / np.sum([np.linalg.norm(c1 - c2) for i, c1 in enumerate(centers) for c2 in centers[i+1:]])
            costs.append(intra + inter)
        return np.array(costs)

    best_cost, best_pos = optimizer.optimize(cost_function, iters=max_iter)
    best_centers = best_pos.reshape(n_clusters, n_features)
    clusters = [[] for _ in range(n_clusters)]
    for idx, sample in enumerate(data):
        distances = [np.linalg.norm(sample - c) for c in best_centers]
        cluster_id = np.argmin(distances)
        clusters[cluster_id].append(idx)
    
    return {
        'labels': np.array([np.argmin([np.linalg.norm(x - c) for c in best_centers]) for x in data]),
        'centers': best_centers,
        'clusters': clusters
    }


def cluster_similarity_matrix(data, clusters):
    """Compute similarity matrix within clusters"""
    ...
    for cluster in clusters:
        ...
        cluster_sim = 1 - pairwise_distances(cluster_data, metric='cosine')
        np.fill_diagonal(cluster_sim, 1)
        for i, idx_i in enumerate(indices):
            for j, idx_j in enumerate(indices):
                sim_matrix[idx_i, idx_j] = cluster_sim[i,j]
...
def predict(args):
    # gpcr-euc_dataset:
    entity_emb = load_embeddings("./data/input_embedding_dict/gpcr/entity_embedding_khalil_dt_gpcr.csv")
    drugs = entity_emb[:188] 
    targets = entity_emb[188:]
    for c1 in np.arange(0.2, 1.0, 0.3):
        for c2 in np.arange(0.2, 1.0, 0.3):
            for c3 in np.arange(0.2, 1.0, 0.3):
                drug_result = fcan_mopso_clustering(drugs, 15, 100,c1,c2,c3)

                target_result = fcan_mopso_clustering(targets, 20, 100,c1,c2,c3)

                drug_sim = cluster_similarity_matrix(drugs, drug_result['clusters'])
                target_sim = cluster_similarity_matrix(targets, target_result['clusters'])
                Mat_Int = np.loadtxt("./data/gpcr-KGE/gpcr-cos_dataset/matbipc.txt", skiprows=1,
                                    usecols=range(1, 95), dtype=float)
                Mat_Sim_DD = drug_sim
                Mat_Sim_TT = target_sim
                Name_T = {line.strip(): i for i, line in
                        enumerate(open('./data/gpcr-KGE/gpcr-cos_dataset/fichier_sortie.txt')) if line.strip()}
                Name_D = {line.strip(): i for i, line in
                        enumerate(open('./data/gpcr-KGE/gpcr-cos_dataset/fichier_colonne.txt')) if line.strip()}
                
                threshold=0.001

                New_DTI = Drug_Target_Interaction_Prediction(c1,c2,c3,'clustering_results-gprc.txt',args=args, Mat_Int=Mat_Int, Mat_Sim_DD=Mat_Sim_DD,
                                                            Mat_Sim_TT=Mat_Sim_TT, Name_D=Name_D, Name_T=Name_T, threshold=0.001)

    return New_DTI


def Drug_Target_Interaction_Prediction(c1,c2,c3,f,args, Mat_Int, Mat_Sim_DD, Mat_Sim_TT, Name_D, Name_T, threshold):
    ...
    for i in range(int(args.cycle)):
        if (args.type == 'SP'):
            ...
            all_scores, all_AUPR, all_AUC, name_DTI = Cross_validation(c1,c2,c3,f,New_v_couple_DTI=New_v_couple_DTI,
...
def Cross_validation(c1,c2,c3,f,New_v_couple_DTI, trees, c, fold_nums=20):
